...
export const ImprovementSuggestionsSchema = SchemaFactory.createForClass(
...
export class AIInterviewResult {
  ...
  // ============ 改进建议 ============
  @Prop({ type: [ImprovementSuggestionsSchema], default: [] })
...
// 创建索引
AIInterviewResultSchema.index({ userId: 1, interviewType: 1, createedAt: -1 });
AIInterviewResultSchema.index({ userId: 1, overallScore: -1 });
AIInterviewResultSchema.index({ userId: 1, status: 1, updatedAt: -1 }); //用于查询未完成的面试
